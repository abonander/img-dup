use config::{ProgramSettings, JsonSettings, PrettyJson, CompactJson, NoJson};
...
fn open_output(settings: &ProgramSettings) -> EitherWriter<File, LineBufferedWriter<StdWriter>> {
    match settings.outfile {
        Some(ref file) => Left(File::create(file).unwrap()),
        None => Right(stdout()),
    }
}

/// Polymorphic `Writer` impl, to get around Rust issue #17322
enum EitherWriter<T, U> {
    Left(T),
    Right(U),
}

impl<T, U> Writer for EitherWriter<T, U> where T: Writer, U: Writer {
    fn write(&mut self, buf: &[u8]) -> IoResult<()> {
        match *self {
            Left(ref mut wrt) => wrt.write(buf),
            Right(ref mut wrt) => wrt.write(buf),
        }
    }
}
